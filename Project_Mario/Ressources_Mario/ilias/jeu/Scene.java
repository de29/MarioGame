...
import ilias.affichage.CompteARebours;
import ilias.affichage.Score;
import ilias.audio.Audio;
import ilias.objets.Bloc;
import ilias.objets.Objet;
import ilias.objets.Piece;
import ilias.objets.TuyauRouge;
import ilias.personnages.Champ;
import ilias.personnages.Mario;
import ilias.personnages.Personnage;
import ilias.personnages.Tortue;
...
public class Scene extends JPanel {
	...
	public Personnage champ1;
	public Personnage champ2;
	public Personnage champ3;
	public Personnage champ4;
	public Personnage champ5;
	public Personnage champ6;
	public Personnage champ7;
	public Personnage champ8;
	
	public Personnage tortue1;
	public Personnage tortue2;
	public Personnage tortue3;
	public Personnage tortue4;
	public Personnage tortue5;
	public Personnage tortue6;
	public Personnage tortue7;
	public Personnage tortue8;
	public Personnage tortue9;
	...
	public Personnage mario;
	
	private ArrayList<Objet> tabObjets; // tableau qui enregistre tous les objets du jeu
	
	private ArrayList<Personnage> tabChamps; // tableau qui enregistre tous les champignons du jeu 
	private ArrayList<Personnage> tabTortues; // tableau qui enregistre toutes les tortues du jeu
	...
	public Scene(){		
		...
		piece1 = new Piece(402, 145,score);
		piece2 = new Piece(1202, 140,score);
		piece3 = new Piece(1272, 95,score);
		piece4 = new Piece(1342, 40,score);
		piece5 = new Piece(1650, 145,score);
		piece6 = new Piece(2650, 145,score);
		piece7 = new Piece(3000, 135,score);
		piece8 = new Piece(3400, 125,score);
		piece9 = new Piece(4200, 145,score);
		piece10 = new Piece(4600, 40,score);
		...
		tabChamps = new ArrayList<Personnage>();	
		...
		tabTortues = new ArrayList<Personnage>();
		...
		// Create instances of the concrete commands
        moveLeftCommand = new MoveLeftCommand(this);
        moveRightCommand = new MoveRightCommand(this);
        jumpCommand = new JumpCommand(this);

        // Create an instance of CommandKeyAdapter and pass the concrete commands
        Clavier keyAdapter = new Clavier(jumpCommand, jumpCommand, jumpCommand);
        keyAdapter.mapCommandToKey(moveLeftCommand, KeyEvent.VK_LEFT);
        keyAdapter.mapCommandToKey(moveRightCommand, KeyEvent.VK_RIGHT);
        keyAdapter.mapCommandToKey(jumpCommand, KeyEvent.VK_SPACE);

        // Add the key adapter as a KeyListener
        this.addKeyListener(keyAdapter);

        // Set the focusable property to true to enable keyboard input
		...
		Thread chronoEcran = new Thread(new Chrono()); // Ajout d'un chronomètre PanJeu
    	...
    	// Detection des contacts de mario avec des pieces
 	 	for(int i = 0; i < this.tabPieces.size(); i++){
 	 		if(this.mario.proche(this.tabPieces.get(i))){
 	 			if(((Mario) mario).contactPiece(this.tabPieces.get(i))){
 		...
 		for(int i = 0; i < this.tabChamps.size(); i++){
 			if(this.tabChamps.get(i).isVivant() == true){
 				...
 				g2.drawImage(this.tabChamps.get(i).die(), this.tabChamps.get(i).getX(), this.tabChamps.get(i).getY() + 20, null); 				
 		...
 		for(int i = 0; i < this.tabTortues.size(); i++){
 			if(this.tabTortues.get(i).isVivant() == true){
 				...
 				g2.drawImage(this.tabTortues.get(i).die(), this.tabTortues.get(i).getX(), this.tabTortues.get(i).getY() + 30, null);
 		...
 		if(Main.scene.mario.isVivant() == true){
 		    if(((Mario) mario).isSaut()){g2.drawImage(((Mario) mario).saute(), this.mario.getX(), this.mario.getY(), null);}
